...
namespace DatelAPI.Areas.Data
{
    public class SageData: ISageData
    {
        ...
        public async Task<Dictionary<string, string>> GetDBLoginDetailsLive(string systemID)
        {
            Dictionary<string, string> Result = new Dictionary<string, string>();
            try
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    ...
                    using (SqlDataReader dr = await _data.OpenQueryCTSLive(cmd))
                    {
                        if (dr.Read())
                        {
                            Result.Add("systemName", dr["systemName"].ToString().Trim());
                            ...
                            Result.Add("slAllowBatchAppend", dr["slAllowBatchAppend"].ToString().Trim());
                            Result.Add("slAllowPartialAllocation", dr["slAllowPartialAllocation"].ToString().Trim());
                            Result.Add("AllowBackOrder", dr["AllowBackOrder"].ToString().Trim());
        ...
        public async Task<bool> StockAllocateAllowed(string order_no)
        {
            bool Result = false;
            try
            {
                ...
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.CommandText = "ESP_Hub_AllocateStockAllowed";

                    cmd.Parameters.Add("order_no", SqlDbType.VarChar).Value = order_no;

                    using (SqlDataReader dr = await _data.OpenQuerySage(cmd))
                    {
                        if (dr.Read())
                        {
                            Result = (dr["Allowed"].ToString() == "1");
                        }
                    }
                }
            }
            ...
            {
                Log.Error(ex, "Error in StockAllocateAllowed");
                throw ex;
            }
            return Result;
